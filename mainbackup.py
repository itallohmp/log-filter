...
def get_logs(limit: int = 50) -> str:
    ...
    with paramiko.SSHClient() as ssh:
        ...
        logs = stdout.read().decode(errors="ignore")
...
def filter_logs(
    ...
    ano: str = "2026", # DECLARAR VARIAVEL PARA RECEBIMENTO NO INDEX
    mes: str = "03", # DECLARAR VARIAVEL NO 
    dia: str = "11",  # DECLARAR VARIAVEL NO 
    ...
    try:
        ...
        if not arquivos_bz and not arquivos_log:
            ...
            return JSONResponse({"erro": "Nenhum log encontrado"}, status_code=404)
        ...
        if not arquivos_log:
            ...
            return JSONResponse({"erro": "Nenhum log encontrado após descompactar"}, status_code=404)
        ...
        return JSONResponse({"erro": f"Erro ao executar script: {str(e)}"}, status_code=500)
    except Exception as e:
